...
const ResidentsList = () => {
  ...
  const fetchResidents = async () => {
    try {
      ...
      const res = await axios.get('https://final-project-al-furqan-rj1r.onrender.com/api/residents', {
...
if (delegateFilterValue.length > 0) {
  ...
  const handleSave = async () => {
    const token = localStorage.getItem('token');
    try {
      await axios.put(`https://final-project-al-furqan-rj1r.onrender.com/api/residents/${formData.id}`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDelete = async (id) => {
    const token = localStorage.getItem('token');
    try {
      await axios.delete(`https://final-project-al-furqan-rj1r.onrender.com/api/residents/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const importFromExcel = (e) => {
    ...
    const token = localStorage.getItem('token');
    axios
      .post('https://final-project-al-furqan-rj1r.onrender.com/api/residents/import', data, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(() => {
